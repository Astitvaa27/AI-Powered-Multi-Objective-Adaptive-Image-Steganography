...
DATA_ROOT = Path(
    "storage/dataset/bossbase"
)

CLEAN_DIR = DATA_ROOT / "clean"

STEGO_DIR = (
    DATA_ROOT / "stego_random_payload"
)
...
def main():
    ...
    for payload in PAYLOAD_LEVELS:
        ...
        for feature in feature_columns:
            ...
            clean_mean = (
                clean_df[feature].mean()
            )

            stego_mean = (
                stego_df[feature].mean()
            )
            ...
            clean_std = (
                clean_df[feature].std()
            )

            stego_std = (
                stego_df[feature].std()
            )
